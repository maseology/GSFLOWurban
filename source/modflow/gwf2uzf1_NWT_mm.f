...
C     ------------------------------------------------------------------
      Version_uzf = 'gwf2uzf1_NWT.f 2016-07-21 11:42:00Z'
...
CDEP 05/05/2006
      ...
      DO ll = 1, NUMCELLS
        ...
        IF ( ibnd.GT.0 ) l = l + 1
! EDM
!gsf    IF ( FINF(ic, ir).GT.VKS(ic, ir) ) THEN
!gsf      EXCESPP(ic, ir) =  (FINF(ic, ir) - 
!gsf +                 VKS(ic, ir))*DELC(ir)*DELR(ic)
!gsf      FINF(ic, ir) = VKS(ic, ir)
!gsf    ELSE
!gsf      EXCESPP(ic, ir) = 0.0
!gsf    ENDIF
! EDM
        finfhold = FINF(ic, ir)
...
C67-----FORMATS.
 9002 FORMAT (1X//,'UNSATURATED ZONE PACKAGE VOLUMETRIC BUDGET FOR ', 
     +        ' TIME STEP', I6, ' STRESS PERIOD ', I5, /2X, 78('-')//) !gsf                                                 ! mm

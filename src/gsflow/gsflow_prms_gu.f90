...
!***********************************************************************
   ...
   &    smidx_module, carea_module, ddsolrad_module, ccsolrad_module, SAVE_INIT, READ_INIT, &                                  !mm
   &    scscn_module, grnampt_module 
      IMPLICIT NONE
...
! Global
      ...
      INTEGER, SAVE :: Transp_flag, Sroff_flag, Solrad_flag, Et_flag, Surban_flag, Gw_flag                                  ! mm
...
! Control parameters
      ...
      CHARACTER(LEN=MAXCONTROL_LENGTH), SAVE :: Strmflow_module, Transp_module, Gw_module
...
! Functions
      ...
      INTEGER, EXTERNAL :: srunoff, soilzone, urban                                                                         ! mm
...
! frost_date is a pre-process module
      ...
      IF ( PRMS_land_iteration_flag==OFF .OR. PRMS_only==ACTIVE) THEN
        ...
        IF ( Surban_flag==ACTIVE ) THEN                                                                                     ! mm
          ierr = urban()                                                                                                    !
          IF ( ierr/=0 ) CALL module_error('urban', Arg, ierr)                                                              !          
        ENDIF                                                                                                               ! mm
      ENDIF
...
!***********************************************************************
      ...
      IF ( Print_debug>DEBUG_less ) THEN                                                                                    ! mm begin
          PRINT 3, GSFLOW_versn
          PRINT 4
      ENDIF                                                                                                                 ! mm end
    ...
    4 FORMAT (/, 19X, 'Urban hydrology edition (GSFLOWurban)', /, &                                                         ! mm
     &        26X, '    Version 0.6.1', /, &                                                                                !
     &        26X, 'copyright M.Marchildon', /)                                                                             ! mm
...
!rsr            kstp = mf_timestep
     ...
     &     (Stream_temp_flag==ACTIVE .AND. Strmtemp_humidity_flag==OFF) ) Humidity_cbh_flag = ACTIVE
      ...
      ELSEIF ( Srunoff_module(:13)=='srunoff_scscn' ) THEN                                                                  ! mm begin
        Sroff_flag = scscn_module
      ELSEIF ( Srunoff_module(:15)=='srunoff_grnampt' ) THEN
        Sroff_flag = grnampt_module
      ...
      ELSEIF ( Srunoff_module(:21)=='srunoff_grnampt_urban' ) THEN
        Surban_flag = ACTIVE
        Sroff_flag = grnampt_module                                                                                         ! mm end
      ELSE
      ...
      IF ( PRMS_flag==1 ) THEN                                                                                              ! mm begin
        ...
        IF ( Gw_module(:11)=='gw_topmodel' ) THEN
          Gw_flag = 2
        ELSEIF ( Gw_module(:6)/='gwflow' ) THEN
          PRINT '(/,2A)', 'ERROR, invalid gw_module value: ', Gw_module
          Inputerror_flag = 1
        ENDIF
      ENDIF                                                                                                                 ! mm
...
! If the Green&Ampt Module is Active, allow for hourly precipitation inputs                                                 ! PJT - 2018Jan03 - Sub-daily precip inputs
      IF ( Sroff_flag == grnampt_module ) THEN                                                                              ! mm: this is safer
        IF ( decldim('nmaxprecipobs', 0, MAXDIM, 'Maximum number of sub-daily precip observations') &                       !
     &       /=0 ) CALL read_error(7, 'nmaxprecipobs')                                                                      ! PJT - 2018Jan03 - Sub-daily precip inputs
      ENDIF 
...
! urban storage facilities                                                                                                  ! mm
      IF ( decldim('ndscn', 1, MAXDIM, 'Number of disconnected reservoirs')/=0 ) CALL read_error(7, 'ndscn')                !
      IF ( decldim('ninfstor', 1, MAXDIM, 'Number of infiltration storage reservoirs')/=0 ) CALL read_error(7, 'ninfstor')  !
...
!***********************************************************************
      ...
      Ndscn = getdim('ndscn')                                                                                               ! mm begin
      IF ( Ndscn==-1 ) CALL read_error(7, 'ndscn')

      Ninfstor = getdim('ninfstor')
      IF ( Ninfstor==-1 ) CALL read_error(7, 'ninfstor')

      IF ( Model/=0 ) THEN
        IF ( Ndscn==0 ) Ndscn = 1
        IF ( Ninfstor==0 ) Ninfstor = 1
      ENDIF                                                                                                                 ! mm end

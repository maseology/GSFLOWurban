...
!***********************************************************************
      ...
      IF ( Mapflg==OFF .OR. Model==DOCUMENTATION ) THEN
        ...
        ALLOCATE ( Map_var_id_sngl(Ngwcell) )                                                                               ! mm
...
!***********************************************************************
      ...
      IF ( Mapvars_freq==8 ) Binresults = ACTIVE                                                                            ! mm
      IF ( Mapvars_freq==7 ) Dailyresults = ACTIVE                                                                          ! mm

      IF ( Mapvars_freq==7 .OR. Mapvars_freq==8 ) THEN                                                                      ! mm
        !Dailyresults = ACTIVE                                                                                              ! mm
        ALLOCATE ( Map_var_daily(Nhru, NmapOutVars), Basin_var_daily(NmapOutVars) )
        ALLOCATE ( Map_var_binary(Nhru, NmapOutVars) )                                                                      ! mm
        Map_var_daily = 0.0D0
...
!***********************************************************************
     ...
     &    End_year, End_month, End_day, Nowyear, Nowmonth, Nowday, Gvr_cell_id, Print_debug                                 ! mm
      USE PRMS_MAP_RESULTS
...
! need getvars for each variable (only can have short string)
     ...
     &         CALL read_error(4, MapOutVar_names(jj)(:Nc_vars(jj)))
      ...
      DO jj = 1, NmapOutVars
        DO j = 1, Active_hrus
          ...
          IF ( Map_var_type(jj)==REAL_TYPE ) THEN
            ...
            ELSEIF ( Binresults==ACTIVE ) THEN                                                                                   ! mm
      ...
      IF ( Binresults==ACTIVE ) THEN                                                                                         ! mm begin

...
!     spatial resolution
! mm modified for hru-binary outputs
!***********************************************************************
...
! Module Variables
      ...
      INTEGER, SAVE :: Begin_results, Begyr, Dailyresults, Binresults                                                       ! mm
      ...
      REAL, SAVE, ALLOCATABLE :: Map_var_id_sngl(:), Map_var_binary(:, :)                                                   ! mm
...
!***********************************************************************
      ...
      IF ( Mapflg==0 .OR. Model==99 ) THEN
        IF ( Nhrucell<1 ) THEN
          ...
          ENDIF
        ENDIF
        ALLOCATE ( Gvr_map_id(Nhrucell), Gvr_map_frac(Nhrucell), Gvr_hru_id(Nhrucell), Map_var_id(Ngwcell), Map_var_id_sngl(Ngwcell) )  ! mm
...
! Declared Parameters
     ...
     &     ' 5=monthly, yearly, and total; 6=weekly; 7=daily; 8=dailybinary)', &                                            ! mm
     ...
     &       'none')/=0 ) CALL read_error(1, 'gvr_cell_id')
        IF ( Nhrucell/=Ngwcell .OR. Model==99 ) THEN                                                                       ! mm
        !IF ( Model==99 ) THEN                                                                                               !
...
!***********************************************************************
      ...
      IF ( Mapvars_freq==8 ) Binresults = 1                                                                                 ! mm
      IF ( Mapvars_freq==7 ) Dailyresults = 1                                                                               ! mm
      
      IF ( Mapvars_freq==7 .OR. Mapvars_freq==8 ) THEN                                                                      ! mm
        !Dailyresults = 1                                                                                                   ! mm
        ALLOCATE ( Map_var_daily(Nhru, NmapOutVars), Basin_var_daily(NmapOutVars), Map_var_binary(Nhru, NmapOutVars) )      ! mm
        Map_var_daily = 0.0D0
        Basin_var_daily = 0.0D0
        Map_var_binary = 0.0                                                                                                ! mm
        ALLOCATE ( Dailyunit(NmapOutVars) )
        IF ( Mapvars_freq==7 ) THEN                                                                                         ! mm
          DO jj = 1, NmapOutVars
        ...
        ELSE                                                                                                                ! mm begin
          DO jj = 1, NmapOutVars
            CALL PRMS_open_output_file(Dailyunit(jj), MapOutVar_names(jj)(:Nc_vars(jj))//'.hru', 'mmBinary', 1, ios)
            IF ( ios/=0 ) ierr = 1
          ENDDO            
        ENDIF                                                                                                               ! mm end
      ENDIF
...
!***********************************************************************
      ...
      USE PRMS_MAP_RESULTS
      USE PRMS_MODULE, ONLY: Nhru, Nhrucell, Start_month, Start_day, End_year, End_month, End_day, Print_debug              ! mm
...
! FUNCTIONS AND SUBROUTINES
      INTRINSIC DBLE, SNGL                                                                                                  ! mm
      INTEGER, EXTERNAL :: getvar
      EXTERNAL read_error, write_results, write_binary                                                                      ! mm
! Local Variables
...
! need getvars for each variable (only can have short string)
     ...
     &         CALL read_error(4, MapOutVar_names(jj)(:Nc_vars(jj)))
      ...
      DO jj = 1, NmapOutVars
        DO j = 1, Active_hrus
          ...
          IF ( Map_var_type(jj)==2 ) THEN
            ...
            ELSEIF ( Binresults==1 ) THEN                                                                                   ! mm
              Map_var_binary(i, jj) = Map_var(i, jj)                                                                        !
          ...
          ELSEIF ( Map_var_type(jj)==3 ) THEN
            ...
            ELSEIF ( Binresults==1 ) THEN                                                                                   ! mm
              Map_var_binary(i, jj) = SNGL( Map_var_dble(i, jj) )                                                           !
            ELSEIF ( Weekresults==1 ) THEN
      ...
      IF ( Binresults==1 ) THEN                                                                                             ! mm begin
          IF ( Print_debug>-1 ) PRINT 9005, '  ', Nowyear, Nowmonth, Nowday
          DO jj = 1, NmapOutVars
            IF ( Mapflg==1 ) THEN
              CALL write_binary(Dailyunit(jj), Map_var_binary(1,jj))
            ELSE
              Map_var_id_sngl = 0.0
              DO k = 1, Nhrucell
                IF ( Gvr_map_frac(k)<NEARZERO ) CYCLE
                Map_var_id_sngl(Gvr_map_id(k)) = Map_var_id_sngl(Gvr_map_id(k)) + &
     &                                           Map_var_binary(Gvr_hru_id(k), jj)*Gvr_map_frac(k)
              ENDDO
              CALL write_binary(Dailyunit(jj), Map_var_id_sngl)
            ENDIF               
          ENDDO
      ENDIF                                                                                                                 ! mm end
...
!***********************************************************************                                                    ! mm begin
!     write_binary - write results to raw binary file
!***********************************************************************
      SUBROUTINE write_binary(Iunit, Values)
      USE PRMS_MAP_RESULTS, ONLY: Numvalues
      IMPLICIT NONE
! Arguments
      INTEGER, INTENT(IN) :: Iunit
...
!***********************************************************************
      WRITE ( Iunit ) Values
      END SUBROUTINE write_binary                                                                                           ! mm end
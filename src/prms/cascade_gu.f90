...
! allocate HRU variables
     ...
     &             'Cascade ignored as down HRU is inactive', i, kup, jdn, frac, istrm
          ...
          ELSEIF ( Cascade_flg==1 ) THEN
            IF ( frac>hru_frac(kup) ) THEN
              ...
              ELSEIF ( jdn>0 ) THEN                                                                                         ! mm begin
                Hru_down(1, kup) = jdn
              ELSE
                Hru_down(1, kup) = 0                                                                                        ! mm end reincorporating farfield                
              ENDIF
            ENDIF
          ELSE
     ...
     &               i, kup, jdn, hru_frac(kup), istrm
            ...
            ELSEIF ( jdn>0 ) THEN                                                                                           ! mm begin
              Hru_down(kk, kup) = jdn
            ELSE
              Hru_down(kk, kup) = 0                                                                                         ! mm end reincorporating farfield
            ENDIF
          ENDIF
        ENDIF
      ENDDO
...
!         Gwr_down_fracwt(k, i) = 0.0
      ...
      DO i = 1, Ncascdgw
        ...
        IF ( istrm>0 .AND. Gwr_type(kup)==3 ) Gwr_type(kup) = 1                                                             ! mm
        IF ( jdn<0 .AND. Gwr_type(kup)==3 ) Gwr_type(kup) = 1                                                               ! mm
          ...
          ELSEIF ( Cascade_flg==1 ) THEN
            IF ( frac>gwr_frac(kup) ) THEN
              ...
              ELSEIF ( jdn>0 ) THEN                                                                                         ! mm begin
                Gwr_down(1, kup) = jdn
              ELSE
                Gwr_down(1, kup) = 0                                                                                        ! mm end reincorporating farfield
              ENDIF
            ENDIF
          ELSE

...
!***********************************************************************
      ...
      IF ( Process_flag==RUN .OR. Process_flag==INIT ) THEN
        ...
        !If sub-daily timesteps are not specified, ensure the DATA file is supplying daily climate data only                        ! PJT begin - 2018Jan03 - Sub-daily precip inputs
        IF (NmaxPrecipObs==0) THEN                                                                                                  ! PJT - 2018Jan03 - Sub-daily precip inputs
            dt = deltim()        !Original check included in code
        ELSE
            dt = 24              !Regardless of the subdaily precip data, this timestep will completed for the entire day
        ENDIF                                                                                                                       ! PJT end - 2018Jan03 - Sub-daily precip inputs

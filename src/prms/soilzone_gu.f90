...
!   Local Variables
      ...
      REAL, SAVE, ALLOCATABLE :: Gwr_to_ssr(:)                                                  ! mm
...
!***********************************************************************
     ...
     &    PRMS_land_iteration_flag, Print_debug                                                                             ! mm
      USE PRMS_SOILZONE
...
! WARNING changing parameter values as invalid for these HRU types, need to be 0 for computations below
        ...
        IF ((Ssr2gw_rate(i)>1.0).AND.(Ssr2gw_exp(i)/=0.0)) THEN                                                             ! mm begin
          IF ( Parameter_check_flag>0 ) THEN
            PRINT *, i, Ssr2gw_rate(i), Ssr2gw_exp(i)
          ELSE
            IF ( Print_debug>-1 ) PRINT *, i, Ssr2gw_rate(i), Ssr2gw_exp(i)
            Ssr2gw_exp(i) = 0.0
          ENDIF
        ENDIF                                                                                                               ! mm end
...
! WARNING!!! Sroff, Basin_sroff, and Strm_seg_in can be updated
     ...
     &    Ssres_stor, Soil_moist, Sat_threshold, Soil_rechr, Basin_sroff, Basin_lake_stor, &                                ! mm
     &    Soil_moist_frac                                                                                                   ! mm
      USE PRMS_INTCP, ONLY: Hru_intcpstor
      USE PRMS_SRUNOFF, ONLY: Hru_impervstor, Dprst_stor_hru
      ...
      USE PRMS_URBAN, ONLY: Urban_to_ssr                                                                                    ! mm
      IMPLICIT NONE
! Functions
...
!******Add infiltration to soil and compute excess
     ...
     &           Soil_moist(i), Soil_rechr(i), Soil_to_gw(i), perv_frac)
            ...
            IF ( Surban_flag==ACTIVE ) gvr_maxin = gvr_maxin + Urban_to_ssr(i)                                              ! mm
          ENDIF
        ENDIF
...
!            Gvr2sm(i) = 0.0
        ...
        ELSE
          IF ( Gw_flag==2 ) gvr_maxin = gvr_maxin + Gwr_to_ssr(i)                                                           ! mm: adding seepage from TOPMODEL
...
! treat dunnianflw as surface runoff to streams
     ...
     &                  ' gravity reservoir is 3*sat_threshold', Slow_stor(i), Sat_threshold(i)
        ...
        IF ( Surban_flag==ACTIVE )  Ssres_in(i) =  Ssres_in(i) + Urban_to_ssr(i)                                            ! mm
...
!******compute interflow
      IF  ( Coef_lin<=0.0 .AND. Coef_sq<=0.0 ) THEN                                                                         ! mm
        Inter_flow = 0.0                                                                                                    !
      ELSEIF ( Coef_lin<=0.0 .AND. Ssres_in<=0.0 ) THEN                                                                     !
        c1 = Coef_sq*Storage
